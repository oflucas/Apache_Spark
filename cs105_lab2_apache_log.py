# Databricks notebook source exported at Wed, 21 Sep 2016 02:24:06 UTC
# MAGIC %md
...
daily_hosts_lists = (daily_hosts_df
                    .map(lambda r: (r[0], r[1]))
                    .collect()
                     )

days_with_hosts = [r[0] for r in daily_hosts_lists]
hosts = [r[1] for r in daily_hosts_lists]
...
avg_daily_req_per_host_df = (total_req_per_day_df.alias('req')
                             .join(daily_hosts_df.alias('host'), 'day')
                             .select(col('day'), (col('req.count') / col('host.count')).alias('avg_reqs_per_host_per_day'))
                             )
...
days_with_avg = days_with_hosts
avgs = [row[1] for row in avg_daily_req_per_host_df.collect()]
...
errors_404_by_day = []
for row in errors_by_date_sorted_df.collect():
...
top_err_date_df = errors_by_date_sorted_df.sort(col('count').desc())
...
hours_with_not_found = range(0, 24)
not_found_counts_per_hour = [row['count'] for row in hour_records_sorted_df.collect()]
